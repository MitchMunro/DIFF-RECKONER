...
diff-reckoner runs live beside a working agent, so every action must feel instant and any unavoidable delay must be signalled — never a silent stale-then-swap. Any change that touches rendering, loading, async data, or the event loop is evaluated for perceived latency and transition quality, not just correctness.
...
- No blocking external call (git) runs on the event-loop or draw thread. Run it on a worker and deliver the result over a channel, so a slow or hung call never freezes input or rendering.
...
- The terminal-editor handoff (`e`) blocks the event loop for the whole editor session. This is a deliberate, permanent exception: the editor owns the pane, and diff-reckoner can neither draw nor read input while it does. A window editor is spawned and never waited on, so the pane keeps answering, and its write lands on the ordinary poll like any other change to the worktree — diff-reckoner adds no watch, wake, or timer for it.
