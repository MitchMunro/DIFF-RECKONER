...
- **No line-comment syntax** (JSON, and Markdown in practice) — refuse, and say
  so in the status bar. Writing an invalid line into a file to hold a note
  breaks the build to store a thought.
...
A small built-in table keyed on file extension. Line comments only. `#` as the
fallback. Extensions with no line comment are the refusal case in §3.4.
