//! Hand a URL or a file to the platform opener, which picks the default app for it: the
//! browser for a clicked link, whatever the OS opens `.md` with for the export file.
//!
...
/// Open `target`, a URL or an absolute path, in its default app via the first available
/// opener. Errors when none is on `PATH` (the caller surfaces it to the status line). The
/// opener hands the target on and exits at once, so this waits for it — reaping the child
/// rather than leaving a zombie, and returning fast enough for a click handler (mirrors the
/// codebase's synchronous tool calls).
pub fn open(target: impl AsRef<OsStr>) -> Result<()> {
    let tool = OPENERS
        .iter()
        .copied()
        .find(|t| crate::proc::on_path(t))
        .context("no opener found (need `open` or `xdg-open`)")?;
    ...
    if !status.success() {
        anyhow::bail!("{tool} failed to open it");
    }
    Ok(())
}
