...
///
/// A window editor hands the file to an instance of its own and returns, so diff-reckoner keeps the
...
/// instance of its own and never reads the terminal, so diff-reckoner keeps it.
...
mod tests {
    ...
    fn window_editors_get_their_line_and_nothing_else() {
        // diff-reckoner adds no flag of its own: the launcher hands the file over and returns, and
        // nothing waits on it.
