...
//!
//! When `$DIFF_RECKONER_LOG` names a writable file, the binary appends one
...
/// Open the log sink from `$DIFF_RECKONER_LOG` if set. Call once at startup.
pub fn init() {
    SINK.get_or_init(|| {
        let path = std::env::var("DIFF_RECKONER_LOG").ok()?;
