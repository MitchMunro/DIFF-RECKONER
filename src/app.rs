...
pub struct App {
    ...
    cache: DiffCache,
    /// The one-slot markdown render memo behind the file tabs' preview. Interior-mutable so
    /// the renderer can fill it from `&App`; cleared with the diff cache on a theme switch.
...
impl App {
    pub fn new(repo: PathBuf, scope: Scope, base: Option<String>) -> Self {
        Self::build(repo, scope, base)
    }

    /// Construct the error-only pane without reading repository state.
    pub(crate) fn blocked(repo: PathBuf, scope: Scope, base: Option<String>) -> Self {
        Self::build(repo, scope, base)
    }

    fn build(repo: PathBuf, scope: Scope, base: Option<String>) -> Self {
    ...
    /// Block the pane on one whole-file configuration failure.
    pub fn set_config_error(&mut self, error: String) {
        ...
        self.cancel_gesture();
        // The search overlay and the find band close when the config view takes over;
    ...
    /// `reveal` re-reveals the cursor when the result lands, for user-initiated switches
    /// only.
    ...
    pub(crate) fn markdown_render(&self, text: &str, width: usize) -> crate::markdown::Rendered {
        let expanded = &self.preview_expanded_details;
    ...
    /// Whether the navigator is hidden.
    #[must_use]
    pub fn navigator_hidden_here(&self) -> bool {
        self.navigator_hidden
    }

    /// to the read pane; showing leaves it there.
    /// to the read pane; showing leaves it there.
...
impl App {
    ...
    /// Open the base picker: one row per branch, the default branch first, the rest by tip
    /// recency, each with its trail facts. Picking the default row is the way back to the
    /// default: its name deletes the pick instead of recording it. A current non-branch pick
    /// is inserted as a row. The highlight opens on the current base, else the first row.
    /// Still opens when that list is empty, so a revision can be typed.
    pub fn open_base_picker(&mut self) {
        ...
        let mut rows: Vec<BaseChoice> = branches
            ...
            .collect();
        // A stable sort, so recency still orders the promoted row and the rest alike.
