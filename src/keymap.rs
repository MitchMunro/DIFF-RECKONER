...
mod tests {
    ...
    fn defaults_bind_every_action_and_hint_is_first_key() {
        ...
        assert_eq!(keymap.action_for(Key::plain('Y')), Some(Action::Copy));
        ...
        assert_eq!(keymap.hint(Action::Copy), Key::plain('y'));
        assert_eq!(keymap.hint(Action::TabAllFiles), Key::plain('2'));
