...
fn enter_on_a_directory_toggles_it() {
    ...
    assert!(app.file_rows.len() < rows, "`enter` collapses an open directory");
    assert_eq!(app.focus, Focus::Files, "and stays in the navigator");
    press(&mut app, &keymap, KeyCode::Enter);
    assert_eq!(app.file_rows.len(), rows, "and expands it again");
}
...
fn a_resting_pointer_keeps_the_arm_but_a_gesture_drops_it() {
    ...
    // pane is not an input the reviewer made, so it must not drop the crossing
    // they armed.
...
fn opening_a_comment_in_files_surfaces_it_under_collapsed_directories() {
    ...
    // `All files` opens with `src/` collapsed; `f` on the card must still reach foo's
    // row and land the cursor on the comment.
    enter_tab(&mut app, Tab::Comments);
    press(&mut app, &Keymap::default(), KeyCode::Char('f'));
...
fn the_footer_offers_edit_file_wherever_the_key_opens_one() {
    ...
    assert!(has(&app), "and still offers the file, since `edit` never takes a comment");
        ...
        app.entries.iter().any(|e| {
            e.path == "here.rs"
                && e.annotation.as_ref().map(|a| a.change)
                    == Some(diff_reckoner::model::ChangeKind::Deleted)
        }),
...
#[test]
fn edit_opens_the_file_under_the_cursor_and_enter_the_comment_when_one_is_there() {
    let r = edited_repo();
    let mut app = app_on(&r);
    let keymap = Keymap::default();
    app.focus = Focus::Diff;
    ...
    // On a commented line `e` still names the file, and `enter` edits the comment.
    comment_on(&mut app, '+', "note");
    press(&mut app, &keymap, KeyCode::Char('e'));
    assert!(!app.composing(), "`e` never opens a comment");
    assert!(app.editor_request.take().is_some(), "it names the file");
    press(&mut app, &keymap, KeyCode::Enter);
    assert!(app.composing(), "`enter` opens the comment under the cursor");
    ...
    press(&mut app, &keymap, KeyCode::Enter);
    assert!(app.composing(), "and `enter` opens a content comment the same way");
...
#[test]
fn the_comments_tab_bar_leads_with_editing_the_selected_comment() {
    use diff_reckoner::app::Tab;
    let r = edited_repo();
    let mut app = app_on(&r);
    comment_on(&mut app, '+', "note");
    ...
    assert_eq!(bands[0], (FooterAction::EditComment, Band::Primary));
    assert!(bands.contains(&(FooterAction::OpenInFiles, Band::Do)));
...
fn the_composer_reserve_keeps_the_anchored_line_visible() {
    ...
    let viewport = 12;
    let effective = viewport - diff_reckoner::ui::composer_height(&app, 80);
...
fn divider_drag_math_and_keyboard_clamps_follow_all_four_positions() {
    ...
    for (position, target_column, target_row) in [
        ...
        let divider = (body.y..body.y + body.height)
            ...
            .find(|&(column, row)| diff_reckoner::ui::hit_divider(area, &app, column, row))
            .unwrap();
        handle_mouse(
            &mut app,
...
fn divider_drag_cancels_until_mouse_up() {
    ...
    let divider = (body.x..body.x + body.width)
        .find(|&col| diff_reckoner::ui::hit_divider(area, &app, col, row))
        .unwrap();
    let heights = vec![1usize; app.visible.len()];
    let event = |kind, column, row| MouseEvent { kind, column, row, modifiers: KeyModifiers::NONE };
...
fn navigator_config_changes_override_only_when_the_config_value_changes() {
    let r = edited_repo();
    let mut app = app_on(&r);
    let default = diff_reckoner::config::PluginConfig::default();
    ...
    let changed = diff_reckoner::config::plugin_config_in(dir.path()).unwrap();
    app.start_divider_drag();
...
fn the_comment_box_grows_as_a_long_line_wraps() {
    ...
    let one_word = diff_reckoner::ui::composer_height(&app, width);
    for ch in "the quick brown fox jumps over the lazy dog again and again".chars() {
        app.input_push(ch);
    }
    let wrapped = diff_reckoner::ui::composer_height(&app, width);
...
fn editing_a_multi_line_comment_opens_the_box_under_its_last_line() {
    ...
    // `enter` on the comment's first line reopens the box right under its last one.
    app.diff_cursor = 0;
    app.activate();
    assert!(app.composing());
...
fn annotation_of(app: &App, path: &str) -> Option<Option<diff_reckoner::file_list::Annotation>> {
    use diff_reckoner::file_list::RowKind;
...
fn all_files_annotates_changed_files_only() {
    use diff_reckoner::app::Tab;
    use diff_reckoner::model::ChangeKind;
    let r = Repo::init();
    r.write("a.rs", "one\n");
    r.write("b.rs", "two\n");
    r.commit_all("init");
...
fn an_oversize_file_in_all_files_degrades_to_a_notice() {
    use diff_reckoner::app::Tab;
    use diff_reckoner::diff::{FileState, View};
    let r = Repo::init();
...
fn mouse(app: &mut App, keymap: &Keymap, kind: MouseEventKind) {
    ...
    handle_mouse(app, event, area, &heights, keymap, &diff_reckoner::export::Clipboard).unwrap();
}
...
fn the_traversal_keys_dispatch_and_rebind() {
    ...
    assert_eq!(app.diff_path.as_deref(), Some("bin.dat"));
    press(&mut app, &rebound, KeyCode::Char('}'));
    assert_eq!(app.diff_path.as_deref(), Some("bin.dat"), "the replaced default is inert");
}

#[test]
fn enter_focuses_the_diff_and_esc_returns_to_the_navigator() {
    let r = edited_repo();
    let mut app = app_on(&r);
    let keymap = Keymap::default();
    app.focus = Focus::Files;
    ...
    app.keys_expanded = true;
    press(&mut app, &keymap, KeyCode::Esc);
    assert_eq!((app.keys_expanded, app.focus), (false, Focus::Diff));
    press(&mut app, &keymap, KeyCode::Esc);
    assert_eq!(app.focus, Focus::Files, "`esc` returns to the navigator");

    // A hidden navigator stays hidden: `esc` never adds a pane.
    press(&mut app, &keymap, KeyCode::Enter);
...
#[test]
fn enter_edits_the_comment_under_the_diff_cursor() {
    let r = edited_repo();
    let mut app = app_on(&r);
    comment_on(&mut app, '+', "note");
    app.diff_cursor = row_with(&app, '+');
    press(&mut app, &Keymap::default(), KeyCode::Enter);
    let Mode::Composing { editing: Some(c) } = &app.mode else { panic!("{:?}", app.mode) };
    assert_eq!(c.text, "note", "`enter` opens the comment under the cursor");
}
...
fn find_match_ranges_is_smart_case_and_non_overlapping() {
    use diff_reckoner::app::find_match_ranges;
...
#[test]
fn the_arrows_select_cards_and_only_enter_opens_one() {
    use diff_reckoner::app::Tab;
    ...
    // `e` names no file here; `enter` opens the selected card, and `↓` in a one-line box
    // stays in it.
    press(&mut app, &keymap, KeyCode::Char('e'));
    assert_eq!((app.edited_card(), app.editor_request.is_none()), (None, true));
    press(&mut app, &keymap, KeyCode::Enter);
    assert_eq!(app.edited_card(), Some(0));
    press(&mut app, &keymap, KeyCode::Down);
    ...
    press(&mut app, &keymap, KeyCode::Char('{'));
    assert_eq!(app.comments.cursor, 0, "`{{` the previous file's first");
...
fn the_comments_tab_acts_through_the_rebindable_keys() {
    ...
    press(&mut app, &keymap, KeyCode::Char('f'));
    assert_eq!(app.tab, Tab::Comments, "a rebound `open-comment` frees `f`");
...
fn the_toggle_carries_the_reading_position_block_aligned() {
    ...
    let theme = diff_reckoner::theme::resolve(Some("catppuccin"));
    let rendered = diff_reckoner::markdown::render(
        doc,
        80,
        &diff_reckoner::highlight::Highlighter::new(theme.syntax),
        &theme.palette,
    );
    let block_start =
...
fn a_result_for_a_view_that_moved_on_is_discarded_whole() {
    ...
    assert!(
        diff_reckoner::land_world_completion(&mut app, stale, 7),
...
fn a_completion_landing_mid_composition_leaves_the_frozen_diff() {
    ...
    assert!(diff_reckoner::land_world_completion(&mut app, early, 9));
...
fn a_reveal_completion_settles_the_tab_and_rearms_the_cursor_reveal() {
    ...
    landing.reveal = true;
    app.reveal_files = false;
    assert!(diff_reckoner::land_world_completion(&mut app, landing, 2));
...
fn outside_a_repo_the_build_yields_the_quiet_empty_snapshot() {
    ...
    assert!(diff_reckoner::world::build_changed(&app.world_input()).unwrap().changed.is_empty());
}
...
fn a_superseded_reveal_rearms_for_the_next_dispatch() {
    ...
    assert!(
        !diff_reckoner::land_world_completion(&mut app, superseded, 4),
...
mod search_overlay {
    ...
    use diff_reckoner::app::{App, Focus, Mode, SearchPhase, Tab};
    use diff_reckoner::keymap::{Keymap, default_keymap};
    use diff_reckoner::land_search_completion;
    use diff_reckoner::search::{
    ...
    fn slash_opens_from_any_tab() {
        ...
        for tab in [Tab::Changes, Tab::AllFiles] {
            enter_tab(&mut app, tab);
            let actions: Vec<_> = app.footer_bands().into_iter().map(|(a, _)| a).collect();
            assert!(
                actions.contains(&diff_reckoner::app::FooterAction::Search),
    ...
    fn flip_keeps_query_and_lands_pick_on_first_row() {
        use diff_reckoner::app::SearchMode;
        let repo = Repo::init();
    ...
    fn footer_offers_only_esc_when_nothing_pickable() {
        use diff_reckoner::app::{Band, FooterAction};
        let repo = Repo::init();
        repo.write("a.rs", "one\n");
        repo.commit_all("c");
        let keymap = default_keymap().clone();
    ...
    fn query_edits_with_comment_editor_controls() {
        ...
        fn key(app: &mut App, keymap: &Keymap, code: KeyCode, mods: KeyModifiers) {
            ...
            diff_reckoner::handle_key(app, KeyEvent::new(code, mods), area, keymap).unwrap();
        }
        let repo = Repo::init();
        repo.write("a.rs", "one\n");
        repo.commit_all("c");
    ...
    fn engine_worker_end_to_end() {
        ...
        let worker =
            diff_reckoner::search::spawn(repo.path_buf(), cache.path().into(), job_rx, res_tx);
...
fn a_one_branch_repo_lists_that_branch() {
    ...
    app.set_scope(Scope::Branch).unwrap();
    assert_eq!(
        app.branch_base.winner.as_ref().map(diff_reckoner::git::ResolvedBase::name),
        Some("main")
    );
...
fn a_refresh_landing_mid_picker_changes_nothing() {
    ...
    let names = |bp: &diff_reckoner::app::BasePicker| -> Vec<String> {
...
fn checking_out_a_picked_branch_does_not_turn_it_into_a_pin() {
    let r = based_repo();
    diff_reckoner::git::write_base_pick(r.path(), "dev").unwrap();
    ...
    assert_eq!(diff_reckoner::git::read_base_pick(r.path()).unwrap().as_deref(), Some("dev"));
}
...
fn sel_mouse(app: &mut App, kind: MouseEventKind, col: u16, row: u16) {
    ...
    let heights = diff_reckoner::ui::diff_row_heights(app, SEL_AREA);
...
fn a_text_drag_maps_tabs_and_wide_chars_and_extracts_source_text() {
    ...
    assert_eq!(diff_reckoner::drag_text(&app, SEL_AREA).as_deref(), Some("beta\n\tif x {\n日本 z"));
...
fn ts_one_surface_a_drag_clamps_to_its_pane_and_copies_a_comment_as_its_line() {
    ...
    let (c0, r0) = sel_cell(&app, 0, 0);
    let files = diff_reckoner::ui::files_inner_rect(SEL_AREA, &app);
    sel_mouse(&mut app, MouseEventKind::Down(MouseButton::Left), c0, r0);
...
fn the_navigator_click_survives_a_one_cell_slip_and_a_row_drag_copies_paths() {
    ...
    let text = diff_reckoner::drag_text(&app, SEL_AREA).unwrap();
...
fn a_release_lost_past_the_border_still_copies() {
    ...
    assert!(
        diff_reckoner::pointer_at_pane_edge(event, SEL_AREA),
    ...
    diff_reckoner::complete_gesture(&mut app, SEL_AREA, &SelClipboard);
    assert!(!app.gesture_active());
...
fn the_drag_h_scroll_caps_at_the_widest_visible_row() {
    ...
    let mut app = app_on(&r);
    app.wrap = false;
    let content = diff_reckoner::ui::read_content_rect(SEL_AREA, &app);
    let widest = diff_reckoner::ui::widest_visible_row(&app, SEL_AREA);
    assert!(widest > 0);
    let (c0, r0) = sel_cell(&app, 0, 0);
    sel_mouse(&mut app, MouseEventKind::Down(MouseButton::Left), c0, r0);
...
fn picker(app: &App) -> &diff_reckoner::app::CommitPicker {
...
fn a_single_pick_reopens_without_an_anchor_so_k_enter_steps() {
    ...
    press(&mut app, &keymap, KeyCode::Char('j'));
    press(&mut app, &keymap, KeyCode::Enter);
    assert_eq!(app.commit_pick, Some(diff_reckoner::model::CommitPick::single(&shas[1])));
    press(&mut app, &keymap, KeyCode::Char('G'));
...
fn every_other_key_is_inert_inside_the_commit_picker() {
    ...
    for code in [
        ...
        assert_eq!(app.tab, diff_reckoner::app::Tab::Changes);
...
fn an_off_branch_pick_keeps_painting_as_a_row_above_the_list() {
    ...
    press(&mut app, &keymap, KeyCode::Char('G'));
    press(&mut app, &keymap, KeyCode::Enter);
    assert_eq!(app.commit_pick, Some(diff_reckoner::model::CommitPick::single(&shas[3])));
    ...
    assert_eq!(
        ...
        Some(&diff_reckoner::world::PickVerdict::OffBranch)
    );
...
fn a_stale_build_for_a_replaced_pick_is_discarded() {
    ...
    press(&mut app, &keymap, KeyCode::Enter);
    assert_eq!(app.commit_pick, Some(diff_reckoner::model::CommitPick::single(&shas[2])));
    assert!(diff_reckoner::land_world_completion(&mut app, stale, 3));
...
fn a_poll_moves_the_pick_between_the_pick_row_and_the_list() {
    ...
    press(&mut app, &keymap, KeyCode::Char('j'));
    press(&mut app, &keymap, KeyCode::Enter);
    let run = diff_reckoner::model::CommitPick { oldest: shas[1].clone(), newest: shas[2].clone() };
...
fn the_folder_dot_appears_under_a_poll_without_moving_the_cursor() {
    ...
    let marked = |app: &App| {
        matches!(
            ...
            diff_reckoner::file_list::RowKind::Dir { has_change: true, expanded: false, .. }
        )
    };
...
fn completion_for(app: &App, generation: u64) -> diff_reckoner::world::WorldCompletion {
    let input = app.world_input();
    let snapshot = diff_reckoner::world::build(&input).unwrap();
    diff_reckoner::world::WorldCompletion {
        generation,
        input,
        reveal: false,
